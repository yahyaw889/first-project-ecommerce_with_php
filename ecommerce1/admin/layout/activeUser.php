...
if(isset($_SESSION['Manger'])){
    $allUsers = $conn->prepare("SELECT * FROM users WHERE statusReg = 0 AND groupID != 1 AND groupID != 2 ORDER BY date DESC");   
    }else{
$allUsers = $conn->prepare("SELECT * FROM users WHERE statusReg = 0 ORDER BY date DESC"); 
    }
    $allUsers->execute();
    ...
    ?>
              ...
              ?> </td>
                ...
                <td>
                    ...
                    <?php }?>
                    <a class="confirm" href="../changes/active.php?id=<?=$row['userID'] ?>"><button
                            data-bs-toggle="tooltip" data-bs-title="Active user" data-bs-placement="top"
...
<?php
        ...
        if(!isset($_SESSION['Manger'])){
            $allUsers = $conn->prepare("SELECT * FROM users WHERE username LIKE '%$search%'  AND statusReg = 0 ");   
            }else{
                $allUsers = $conn->prepare("SELECT * FROM users WHERE username LIKE '%$search%' AND statusReg = 0 AND groupID != 1 AND groupID != 2");
            }
        $allUsers->execute();
        $rows = $allUsers->fetchAll();
?>
    ...
    <nav class="navbar bg-dark my-3">
        <div class="container-fluid justify-content-center justify-content-md-between">
            ...
            <form class="d-flex" role="search" method="get"
                action="?search=<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>">
                <input name="search" class="form-control me-2" type="search" placeholder="<?= lang("search") ?>"
                    aria-label="Search"
                    value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>" />
                <button class="btn btn-outline-success" type="submit">
                ...
                <a href="?search=<?= '' ?>"> <button class="btn mx-2 btn-danger" type="button">
                        STOP
                    </button></a>
            </form>
        </div>
