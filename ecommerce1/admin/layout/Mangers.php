...
if($search == ''){
    $allUsers = $conn->prepare("SELECT * FROM users WHERE statusReg != 0 AND groupID = 2");
    $allUsers->execute();
...
<?php
        ...
        $allUsers = $conn->prepare("SELECT * FROM users WHERE username LIKE '%$search%' AND statusReg != 0 AND groupID = 2");
        $allUsers->execute();
        $rows = $allUsers->fetchAll();
?>
